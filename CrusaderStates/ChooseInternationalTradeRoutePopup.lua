...
local CIVILIZATION_JERUSALEM = assert(
	...
	"Holy Land route picker: Jerusalem civilization is missing"
);
local PANTHEON_RELIGION = assert(
	ReligionTypes.RELIGION_PANTHEON,
	"Holy Land route picker: pantheon religion is missing"
);
...
function RefreshData()
	...
	for i,v in ipairs(potentialTradeSpots) do
		...
		if (hasHolyLandBonus) then
			table.insert(
				myBonuses,
				"[ICON_ARROW_LEFT] " .. Locale.Lookup(
					"TXT_KEY_JERUSALEM_HOLY_LAND_ROUTE_PICKER_BONUS"
				)
			);
		end
